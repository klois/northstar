...
pub enum Error {
    ...
    SetupNetNamespace {error: i32, str: String, subnet: i32 },
...
impl Minijail {
    ...
    pub fn setup_net_namespace(&mut self, strarg: &str, subnet: libc::c_int) -> Result<()> {
        ...
        let libc_strarg =
            CString::new(strarg).map_err(|_| Error::StrToCString(strarg.to_owned()))?;
        unsafe {
            ret = minijail_setup_net_namespace(self.jail, libc_strarg.as_ptr(), subnet)
        }
        if ret != 0 {
    ...
    pub fn init_unlink_net_namespace(&mut self) -> Result<()> {
        ...
        if ret != 0 {
            return Err(Error::InitUnlinkNetNamespace {
                error: ret,
            });
        }
        return Ok(());
    ...
    pub fn remove_net_bridge() -> Result<()> {
        ...
        if ret != 0 {
            return Err(Error::RemoveNetBridge {
                error: ret,
            });
        }
        return Ok(());
